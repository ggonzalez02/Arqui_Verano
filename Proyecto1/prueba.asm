...
section .data
	...
	ball_pos_x dq 20          ; Posición X inicial
    ball_pos_y dq 15          ; Posición Y inicial
    ball_dir_x dq 1           ; Dirección X
    ball_dir_y dq -1          ; Dirección Y
    last_ball_pos_x dq 20     ; Última posición X
    last_ball_pos_y dq 15     ; Última posición Y
    ball_speed dq 10          ; Control de velocidad
    ball_speed_count dq 0     ; Contador de velocidad
...
; Función para obtener el carácter en una posición
get_char_at_position:
    ; Input: rax = posición Y, rbx = posición X
    ; Output: al = carácter en la posición
    push rdx
    push rdi
    
    mov rdi, column_cells+2
    mul rdi                    ; rax = y * (column_cells+2)
    add rax, rbx              ; rax += x
    add rax, board            ; rax += dirección base del tablero
    mov al, byte [rax]        ; al = carácter en la posición
    
    pop rdi
    pop rdx
    ret

; Función para limpiar la posición anterior de la pelota
clear_ball:
    push rax
    push rbx
    push rcx
    ...
    mov rax, [last_ball_pos_y]
    mov rbx, [last_ball_pos_x]
    call get_char_at_position
    
    ; No limpiar si hay una X
    cmp al, 'X'
    je .skip_clear
    ...
    mul rbx
    add rax, [last_ball_pos_x]
    add rax, board
    
    ; Limpiar solo si no es una X
    mov byte [rax], ' '
...
move_ball:
    ...
    inc qword [ball_speed_count]
    mov rax, [ball_speed_count]
    cmp rax, [ball_speed]
    jl .end
    mov qword [ball_speed_count], 0
    
    ; Guardar posición actual
    mov rax, [ball_pos_x]
    mov [last_ball_pos_x], rax
    mov rax, [ball_pos_y]
    mov [last_ball_pos_y], rax
    
    ; Verificar próxima posición X
    mov rax, [ball_pos_x]
    add rax, [ball_dir_x]
    cmp rax, 2                ; Límite izquierdo
    jle .bounce_x
    cmp rax, column_cells-3   ; Límite derecho
    jge .bounce_x
    
    ; Mover en X si es seguro
    mov [ball_pos_x], rax
    jmp .check_y
    
.bounce_x:
    neg qword [ball_dir_x]
    
.check_y:
    ; Verificar próxima posición Y
    mov rax, [ball_pos_y]
    add rax, [ball_dir_y]
    cmp rax, 2                ; Límite superior
    jle .bounce_y
    cmp rax, row_cells-4      ; Límite inferior
    jge .check_paddle
    
    ; Verificar si la próxima posición tiene una X
    push rax
    mov rbx, [ball_pos_x]
    call get_char_at_position
    cmp al, 'X'
...
.check_paddle:
    ...
    mov rbx, [pallet_position]
    sub rbx, board
    xor rdx, rdx
    mov rax, rbx
    mov rcx, column_cells+2
    div rcx
    mov rbx, rdx              ; rbx = posición X de la paleta
...
.reset_ball:
    ...
    mov qword [ball_pos_x], 20
    mov qword [ball_pos_y], 15
    mov qword [ball_dir_y], -1
    dec qword [current_lives]
...
print_ball:
    ...
    mov rbx, [ball_pos_x]
    call get_char_at_position
    cmp al, 'X'               ; No dibujar si hay una X
    je .skip_draw
...
_start:
    ...
    mov qword [ball_pos_x], 15
    mov qword [ball_pos_y], 15
    mov qword [ball_dir_x], 1
    mov qword [ball_dir_y], 1
    mov qword [last_ball_pos_x], 15
    mov qword [last_ball_pos_y], 15
